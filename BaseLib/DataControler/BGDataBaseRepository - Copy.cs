...
namespace BotGuruz.BusinessSitesScrappers.DataControler
{
    public class BGDataBaseRepository
    {
            ...
            try
            {
               ...
               
                {
                    ...
                    int rowAffected = conn.Execute(@"INSERT INTO Business_Data_TableMain(Site_Id,Category_Id,SubCategory_Id,SubCategory2,SubCategory3,SubCategory4,SubCategory5,ScrapId,City,State,Business_Name,Slogen,Address,ZipCode, " +
                                                           ...
                                                           new
                                                           {Site_Id = SiteId,Cat_Id = CatId,SCat_Id = SCatId,Scat2 = Scat2,
                                                               Scat3 = Scat3,
                                                               Scat4 = Scat4,
                                                               Scat5 = Scat5,
                                                               Scrp_Id = ScrapId,
        ...
        public static int InsertData(string name, string Url)
        {
            try
            {
                ...
                {
                    conn.Open();
                    int rowAffected = conn.Execute(@"INSERT into primarycateogry (cateogryName , url) VALUES ( '" + name + "'  , '" + Url + "' )");
                    return rowAffected;
                }
            }
            catch (Exception ex)
        ...
        public static int SelectAllDataManta()
        {
            try
            {
                ...
                {
                    con.Open();
                    string query = "SELECT COUNT(*) FROM MantaData";
            ...
            try
            {
                ...
                using (MySqlConnection con = new MySqlConnection(BG_Db_Class.getNewconnectionstring()))
                {
                    ...
                    using (MySqlCommand cmd = new MySqlCommand(query, con))
                    {
                        ...
                        {
                            query = "Select * FROM MantaData LIMIT " + PageSize + " ";
                        }
                        else
                        {
                            query = "Select * from MantaData LIMIT " + CurrentPageIndex + " , " + PageSize + " "; // +
                        }
                    }

                    using (MySqlDataAdapter adapter = new MySqlDataAdapter(query, con))
                    {
                        adapter.Fill(ds, "MantaData");
                    }
                }
                return ds;
            }
